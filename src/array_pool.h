...
//		struct Allocation
//		{
//			explicit Allocation(T* data, size_t size) : data(data), size(size) {}
//
...
//
//			if (auto it = pool.lower_bound(size); it != pool.end())
//			{
...
//			}
//
//			Allocation alloc(std::make_shared<T[]>(size), size);
//			alloc.requested_size = size;
//			return alloc;
//		}
//
...
//			std::scoped_lock<std::mutex> lock(mutex);
//
//			pool[allocation.size] = allocation;
//		}
//
...
//	private:
//		std::mutex mutex;
//		std::map<size_t, Allocation> pool;
//	};
//}
